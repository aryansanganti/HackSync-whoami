-- =====================================================
-- CIVIC AI - COMPREHENSIVE SUPABASE SCHEMA
-- =====================================================
-- This schema supports all features of the Civic AI application
-- including issues, community features, notifications, AI analysis,
...
-- Issue statistics (materialized view for performance)
...
-- Insert storage bucket configurations
...
CREATE POLICY "Users can read their own notifications" ON notifications FOR SELECT USING (auth.uid() IN (
  SELECT user_id FROM user_profiles WHERE user_id = notifications.user_id
));
CREATE POLICY "Users can update their own notifications" ON notifications FOR UPDATE USING (auth.uid() IN (
  SELECT user_id FROM user_profiles WHERE user_id = notifications.user_id
));
...
REFRESH MATERIALIZED VIEW issue_stats;
...
CREATE INDEX CONCURRENTLY idx_issues_active ON issues(created_at DESC) WHERE status != 'Resolved';
CREATE INDEX CONCURRENTLY idx_issues_unassigned ON issues(created_at DESC) WHERE assigned_to IS NULL;
CREATE INDEX CONCURRENTLY idx_notifications_unread ON notifications(created_at DESC) WHERE is_read = false;

-- Create composite indexes for common queries
CREATE INDEX CONCURRENTLY idx_issues_status_category ON issues(status, category);
CREATE INDEX CONCURRENTLY idx_issues_reporter_status ON issues(reporter_id, status) WHERE reporter_id IS NOT NULL;
