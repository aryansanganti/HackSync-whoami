...
export default function OfficerScreen() {
  ...
  useEffect(() => {
    ...
    const subscription = SupabaseService.subscribeToIssues((payload) => {
      ...
      loadIssues(); // Reload issues when there's a change
    });
  ...
  const loadIssues = async () => {
    try {
      // Officers should see all issues including anonymous ones
                  ...
                  <View style={{ flexDirection: 'row', gap: 8 }}>
                    {(['Pending', 'In Progress', 'Resolved'] as IssueStatus[]).map(status => (
                      <TouchableOpacity
                        ...
                        style={{
                          flex: 1,
                          backgroundColor: selectedIssue.status === status
                      ...
                      >
                        <Text style={{
                          color: selectedIssue.status === status
                            ? '#ffffff'
